"""Integration tests for PosEGNNCalculator.

PosEGNNCalculator requires a model checkpoint path.
Set the POSEGNN_MODEL env var to the model path before running these tests.
"""
...
from materialsframework.calculators.posegnn import PosEGNNCalculator

MODEL_PATH = os.environ.get("POSEGNN_MODEL", "")
...
@pytest.mark.integration
def test_calculate_energy(calc: PosEGNNCalculator, bcc_fe) -> None:
...
@pytest.mark.integration
def test_calculate_forces_shape(calc: PosEGNNCalculator, bcc_fe) -> None:
...
@pytest.mark.integration
def test_relax_returns_structure(calc: PosEGNNCalculator, bcc_fe) -> None:
